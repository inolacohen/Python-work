    ...
    9
      ...
      6
        ...
        5
          ...
          2
    """
    if is_weight(m):       # base case is just the weight, returning the root of weight
        return tree(size(m), [])
...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t) and label(t) == old:
            return tree(new, [tree(b) for b in branches(t)])
...
def make_fib():
    ...
    def fib_number():
        nonlocal left_num, right_num, fibonacci_num

        if fibonacci_num == -1 or fibonacci_num == 0:
            fibonacci_num += 1

        else:
            fibonacci_num = left_num + right_num
            left_num, right_num = right_num, fibonacci_num
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, pass_word):
        ...
        if pass_word == password and len(incorrect_attempts) < 3:
            if amount > balance:
